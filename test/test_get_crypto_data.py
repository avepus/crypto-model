...
class TestMain(unittest.TestCase):
    ...
    def test_main_retrive_and_stored(self):
        ...
        with patch('rba_tools.retriever.get_crypto_data.retrievers.CSVDataRetriever.fetch_ohlcv', return_value=constants.empty_ohlcv_df_generator()):
    ...
    def test_main_multiple_timeframe(self):
        ...
        csv_puller_1h.fetch_df(symbol, timeframe_str_1h, from_date_str, to_date_str)
        csv_puller_1d.fetch_df(symbol, timeframe_str_1d, from_date_str, to_date_str)
    ...
    def test_main_data_store_retrieve_prior(self):
        ...
        csv_puller.fetch_df(symbol, timeframe_str, partial_from_date_str, partial_to_date_str)
        #retrieve the stored data while having multiple timeframes in that date range
    ...
    def test_main_data_store_retrieve_prior_verify_call(self):
        ...
        csv_puller.fetch_df(symbol, timeframe_str, partial_from_date_str, partial_to_date_str)
        #retrieve the stored data while having multiple timeframes in that date range
        puller.online_pull = MagicMock(return_value=constants.empty_ohlcv_df_generator())
        puller.fetch_df(symbol, timeframe_str, full_from_date_str, full_to_date_str)
